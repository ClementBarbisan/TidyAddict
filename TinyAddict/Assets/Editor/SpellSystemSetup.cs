...
namespace Projectiles
{
    public static class SpellSystemSetup
    {
        ...
        {
            if (System.IO.Directory.Exists(System.IO.Path.Combine(Application.streamingAssetsPath, VoskModelFolder)) ==
                false)
            {
                Debug.LogWarning(
                    $"[SpellSystemSetup] Modèle Vosk introuvable : StreamingAssets/{VoskModelFolder} — la reconnaissance vocale ne marchera pas.");
            }
        ...
        private static void ConfigureVoskPlugins()
        {
            ...
            foreach (string dll in new[]
                         { "libvosk.dll", "libstdc++-6.dll", "libwinpthread-1.dll", "libgcc_s_seh-1.dll" })
            {
        ...
        private static NetworkObject CreateSpellWallPrefab()
        {
            ...
            try
            {
                ...
                {
                    Debug.LogWarning(
                        $"[SpellSystemSetup] Modèle introuvable : {WallModelPath} — cube de secours utilisé.");
        ...
        private static Material GetOrCreateStoneMaterial()
        {
            ...
            var albedo =
                AssetDatabase.LoadAssetAtPath<Texture2D>(
                    "Assets/Environnement/Enviro.fbm/TCom_TileStones_2K_albedo.tif");
            var normal =
                AssetDatabase.LoadAssetAtPath<Texture2D>(
                    "Assets/Environnement/Enviro.fbm/TCom_TileStones_2K_normal.tif");
        ...
        private static GameObject CreateScrollPrefab()
        {
            ...
            try
            {
                ...
                var rollMaterial = GetOrCreateMaterial(ParchmentMaterialPath, new Color(0.87f, 0.78f, 0.58f),
                    emissive: false);
        ...
        private static void SetupPlayerPrefab(NetworkObject ballPrefab, NetworkObject iceZonePrefab,
            NetworkObject wallPrefab, NetworkObject blackHolePrefab)
        {
        ...
        private static void SetupScene(GameObject scrollPrefab, NetworkObject gameStatePrefab)
        {
            ...
            if (runner != null && runner.GetComponent<Recorder>() != null)
            {
                ...
                spawnerSo.FindProperty("_scrollPrefab").objectReferenceValue =
                    scrollPrefab.GetComponent<NetworkObject>();
            ...
            {
                Debug.LogWarning(
                    "[SpellSystemSetup] Runner avec Recorder introuvable — lancez d'abord le setup du vocal spatial.");
            }
            ...
            var sceneScrolls =
                Object.FindObjectsByType<SpellScroll>(FindObjectsInactive.Include, FindObjectsSortMode.None);
            ...
            if (sceneScrolls.Length > 0)
                Debug.Log(
                    $"[SpellSystemSetup] {sceneScrolls.Length} parchemins de scène supprimés (remplacés par le SpellScrollSpawner).");
        ...
        {
            var existingPoints =
                Object.FindObjectsByType<ScrollSpawnPoint>(FindObjectsInactive.Include, FindObjectsSortMode.None);
            ...
            if (created > 0)
                Debug.Log(
                    $"[SpellSystemSetup] {created} points de spawn de parchemins ajoutés ({total} au total) — déplacez-les librement.");
        }
        ...
        private static void CreateZoneStepPoints()
        {
            ...
            Debug.Log(
                "[SpellSystemSetup] 6 points d'étapes de zones créés (3 rouges, 3 bleus) — déplacez-les librement.");
        }
        ...
        private static Material GetOrCreateZoneXRayMaterial(string path, Color color)
        {
            ...
            {
                Debug.LogWarning(
                    "[SpellSystemSetup] Shader TidyAddict/ZoneXRay introuvable — matériau transparent standard utilisé.");
